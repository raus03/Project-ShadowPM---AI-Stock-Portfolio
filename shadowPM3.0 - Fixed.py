...
# This should be always less than the year we are predicting for
final_pred_yr = 2023
...
#when we don't put 2019 we don't get the prediction of 2020
predictionYears = [2021,2022,2023]
...
def predictingReturns(model, scaler, df_predictionYear, year):
    ...
    print("SECTOR column from df_predictionYear is: ",X_sector)
    X = df_predictionYear.drop(columns = ['1Y_FORWARD_RETURNS', 'DATE', 'NEXT_PX', 'STOCK_NAME', 'SECTOR'], inplace = False)

    cols = X.columns
    print("df_predictionYear all coumns after dropping 1Y_FORWARD_RETURNS, DATE, NEXT_PX, STOCK_NAME, SECTOR ",cols)
...
'''
If final_pred_year != prediction_year + 1, then this function is called to redefine the input-output.
Takes in 12 month lag data. 

Suppose final_pred_year --> 2022 and prediction_year --> 2020.

12 month lag contains entries till 2021 (with 1Y Forward returns calculated on future year) --> df_model
Input arguments are df_model, prediction_year+1, stocks to drop.

For prediction, it takes data of Jan 2021 and drops stocksToDrop.
For training, it takes data before 2021 and again drops stocksToDrop.
'''
...
def yearWiseData(df, year, stocksToDrop):
    ...
    # Drops stocks which were found using 'drop_stock' function
    df_predictionYear = df_predictionYear[~df_predictionYear['STOCK_NAME'].isin(stocksToDrop)]
    
    # Takes all data before prediction year
...
def nav(df_pred, ascending, sheetname, year):
  ...
  excel_book = pxl.load_workbook(os.path.join(directory, "nav3.0.xlsx"))
  
  with pd.ExcelWriter(os.path.join(directory, "nav3.0.xlsx"), engine='openpyxl') as writer:
...
#df_model = df_model[df_model['DATE'].dt.year > 2018]
...
df_predict.to_excel(f"Test_Datafor.xlsx")
...
allPrices.drop(allPrices.columns[0], axis = 1, inplace = True)
...
wb = pxl.Workbook()
wb.save(filename=os.path.join(directory, "nav3.0.xlsx"))
...
with pd.ExcelWriter(os.path.join(directory, 'PredictedReturnsYearWiseV3.xlsx'), engine='openpyxl') as writer:
    for year in predictionYears: 
        
        # Calculating stocks to drop
        drop_stock(year)
        ...
        df_predictionYear = df_predictionYear.to_excel(f"Test_Datafor{year}.xlsx")
        df_trainingYears.to_excel(f"Training_Datafor{year}.xlsx")
        ...
        if year != final_pred_yr:
            ...
            estimateModelPerformance(model, X_val, Y_val)
